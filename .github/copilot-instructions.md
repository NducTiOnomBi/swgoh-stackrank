...
```json
{
    ...
    "excludeIfPresentCharacters": ["BADBATCHOMEGA"], // Optional: character IDs that prevent synergy activation if present
    "categoryDefinitions": [
...
7. **Character References**: All character IDs in `synergySets[].characters` and `synergySets[].excludeIfPresentCharacters` arrays MUST reference existing characters in the data file
